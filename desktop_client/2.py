...
DATA_HISTORY_LEN = 500 # Data points for calculations
STATS_HISTORY_LEN = 200 # Data points for latency stats
    ...
    while app_instance.is_running:
        try:
            ...
            if abs(votes) >= params['vote_threshold']:
                ...
                # --- NEW: Check inventory limits before sending signal ---
                current_inv_usd = app_instance.inventory[symbol] * price
                trade_size = params.get(f'max_pos_{symbol}', 500000.0)
                if signal_type == "BUY" and current_inv_usd + trade_size > trade_size:
                    pass # Block buy if it exceeds max position
                else:
                    app_instance.signal_queue.put({"type": signal_type, "price": price, "symbol": symbol, "reason": f"Vote: {votes}", "size_usd": trade_size})
            
            # --- Latency Reporting ---
            if origin_ts > 0:
                latency_ms = (time.perf_counter() - origin_ts) * 1000
                app_instance.stats_queue.put({'type': 'latency', 'value': latency_ms})
...
class QuantTerminal:
    ...
    def _build_ui(self):
        ...
        # --- NEW: INVENTORY MANAGEMENT PANEL ---
