...
# --- ZMQ BACKGROUND WORKER (Minor change for shutdown) ---
def zmq_thread_worker(app_instance):
    ...
    # ... (connection and subscription logic is the same) ...
    for symbol in SYMBOLS:
        socket.setsockopt_string(zmq.SUBSCRIBE, f"TRADE_{symbol}")
    
    price_history = {s: pd.DataFrame(columns=['price']) for s in SYMBOLS}
    print("ZMQ thread connected and listening...")

    # --- CHANGE: Loop only while the app is running ---
    while app_instance.is_running:
        try:
            # Use a poll with a timeout to make the loop check the is_running flag
            if socket.poll(timeout=100): 
                ...
                price_history[symbol] = pd.concat([price_history[symbol], new_row], ignore_index=True)
                if len(price_history[symbol]) > DATA_HISTORY_LEN:
                    price_history[symbol] = price_history[symbol].iloc[-DATA_HISTORY_LEN:]
                
                params = app_instance.get_params()
                latest_indicators = {}
                if len(price_history[symbol]) > params['long_window']:
                    short_sma = price_history[symbol]['price'].rolling(window=params['short_window']).mean().iloc[-1]
                    long_sma = price_history[symbol]['price'].rolling(window=params['long_window']).mean().iloc[-1]
                    latest_indicators['short_sma'] = short_sma
                    latest_indicators['long_sma'] = long_sma
                    rsi = ta.rsi(price_history[symbol]['price'], length=params['rsi_period']).iloc[-1]
                    latest_indicators['rsi'] = rsi
                    bbands = ta.bbands(price_history[symbol]['price'], length=params['bb_period'], std=params['bb_std'])
                    latest_indicators['bbl'] = bbands[f'BBL_{params["bb_period"]}_{params["bb_std"]}'].iloc[-1]
                    latest_indicators['bbu'] = bbands[f'BBU_{params["bb_period"]}_{params["bb_std"]}'].iloc[-1]
                
                votes = 0
                if params['ma_enabled'] and 'short_sma' in latest_indicators:
                    if latest_indicators['short_sma'] > latest_indicators['long_sma']: votes += 1
                    else: votes -= 1
                if params['rsi_enabled'] and 'rsi' in latest_indicators:
                    if latest_indicators['rsi'] < params['rsi_oversold']: votes += 1
                    elif latest_indicators['rsi'] > params['rsi_overbought']: votes -= 1
                if params['bb_enabled'] and 'bbu' in latest_indicators:
                    if price > latest_indicators['bbu']: votes += 1
                    elif price < latest_indicators['bbl']: votes -= 1
                ...
                if votes >= params['vote_threshold']:
                    app_instance.signal_queue.put({"type": "BUY", "price": price, "symbol": symbol, "reason": f"Conviction Score: {votes}"})
                elif votes <= -params['vote_threshold']:
                    app_instance.signal_queue.put({"type": "SELL", "price": price, "symbol": symbol, "reason": f"Conviction Score: {votes}"})
        except zmq.Again:
...
class TradingApp:
    def __init__(self, root):
        ...
        self.is_running = True # --- NEW: Flag to control the background thread ---
    ...
    def on_closing(self):
        ...
        self.is_running = False # Signal the ZMQ thread to stop
    ...
    def _build_ui(self):
        ...
        style.configure("Treeview.Heading", font=("Consolas", 10, "bold")); style.configure("Treeview", rowheight=25, font=("Consolas", 10))
        ...
        for col in self.grid_cols: self.tree.heading(col, text=col); self.tree.column(col, width=100, anchor='w')
        for symbol in SYMBOLS: self.tree.insert("", "end", iid=symbol, values=[symbol] + ["-"] * (len(self.grid_cols) - 1))
        self.tree.pack(fill=tk.BOTH, expand=True); self.tree.bind('<<TreeviewSelect>>', self._on_symbol_select)
        log_label = tk.Label(grid_frame, text="Trade Log", font=("Arial", 12, "bold"), pady=5); log_label.pack(fill=tk.X)
        self.log = scrolledtext.ScrolledText(grid_frame, height=10, font=("Consolas", 9), state='disabled'); self.log.pack(fill=tk.X, expand=False, pady=(0, 5))
        ...
        self.selected_symbol_label = tk.Label(tab_charts, text="<Select a symbol>", font=("Consolas", 12, "bold")); self.selected_symbol_label.pack(pady=5)
        self.fig_price, self.ax_price = plt.subplots(tight_layout=True); self.canvas_price = FigureCanvasTkAgg(self.fig_price, master=tab_charts); self.canvas_price.get_tk_widget().pack(fill=tk.BOTH, expand=True)
    ...
    def _update_gui(self):
        if not self.is_running: return # Stop updates if shutting down
        # ... (rest of function is the same) ...
        while not self.data_queue.empty():
            data = self.data_queue.get_nowait(); self._process_data_update(data)
        while not self.signal_queue.empty():
            signal = self.signal_queue.get_nowait(); self._process_signal(signal)
        self._update_portfolio_display()
        selected = self.tree.selection(); 
        if selected: self._update_chart(selected[0])
        self.root.after(200, self._update_gui)
    
    # All other methods (_process_data_update, _process_signal, _update_portfolio_display, 
    # _on_symbol_select, _update_chart, _update_pnl_chart, log_message) remain the same
    # as the previous full code block. Copy them here.
    def _process_data_update(self, data):
        ...
        if self.tree.exists(symbol):
            ...
            rsi = indicators.get('rsi', 50); rsi_vote = (1 if rsi < p['rsi_oversold'] else -1 if rsi > p['rsi_overbought'] else 0) if p['rsi_enabled'] else 0
    ...
    def _process_signal(self, signal):
        ...
        if signal_type == "BUY" and self.cash >= TRADE_SIZE_USD:
            self.inventory[symbol] += trade_qty; self.cash -= TRADE_SIZE_USD; self.log_message(f"  -> EXECUTED BUY of {trade_qty:.4f} {symbol}", tag="buy")
        elif signal_type == "SELL" and self.inventory[symbol] >= trade_qty:
            self.inventory[symbol] -= trade_qty; self.cash += TRADE_SIZE_USD; self.log_message(f"  -> EXECUTED SELL of {trade_qty:.4f} {symbol}", tag="sell")

    def _update_portfolio_display(self):
        inventory_value = sum(self.inventory[s] * self.latest_prices[s] for s in SYMBOLS)
    ...
    def _update_chart(self, symbol):
        ...
        if len(prices) > 1:
            ...
            min_p, max_p = min(prices), max(prices); p_range = max_p - min_p if max_p > min_p else max_p * 0.01
            self.ax_price.set_ylim(min_p - p_range * 0.1, max_p + p_range * 0.1)
            buy_signals = [s for s in signals if s['type'] == 'BUY']
            sell_signals = [s for s in signals if s['type'] == 'SELL']
            if buy_signals: self.ax_price.scatter(len(prices) - 1, buy_signals[-1]['price'], color='green', marker='^', s=100, label='Buy Signal', zorder=5)
            if sell_signals: self.ax_price.scatter(len(prices) - 1, sell_signals[-1]['price'], color='red', marker='v', s=100, label='Sell Signal', zorder=5)
        self.ax_price.set_title(f'{symbol} Price Action'); self.ax_price.grid(True, linestyle='--', alpha=0.6); self.ax_price.legend(fontsize='small')
...
if __name__ == '__main__':
    root = tk.Tk()
    app = TradingApp(root)
    # The mainloop will be stopped by the on_closing method
    root.mainloop()